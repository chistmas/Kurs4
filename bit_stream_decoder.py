...
def decoder(bit_stream, red_stream_decoder, green_stream_decoder, blue_stream_decoder, file_name):
    code_stream = bit_stream_decode(file_name)
    if code_stream == []:
        while code_search(bit_stream, red_stream_decoder, 5) != 'separator':
        ...
        while code_search(bit_stream, green_stream_decoder, 5) != 'separator':
...
def bit_stream_decode(file_name):
    file_to_be_decoded = cv2.imread(file_name)
